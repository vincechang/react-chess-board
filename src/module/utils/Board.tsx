...
export class Board extends Fen {
  ...
  public get(positionOrCoordinate: PositionOrCoordinate): BoardContent {
    ...
    if (boardContent === null) {
      throw new Error(
        `${Board.toCoordinate(positionOrCoordinate)} is outside the board.`,
      )
    }
  ...
  public isCapture(fromPosition: Position, toPosition: Position) {
    ...
    return (
      (target !== null && target !== BoardContent.EmptySquare) ||
      (piece.includes(Piece.Pawn) &&
        toPosition.toCoordinate() === this.enPassantSquare)
    )
  }
  ...
  public isPawnMove(fromPosition: Position, toPosition: Position): boolean {
    ...
    if (this.toMove === 'white' && piece === BoardContent.WhitePawn) {
      return (
        fromPosition.isNorthTo(toPosition) &&
        isPawnMove(fromPosition.y === this.rows - 2)
      )
    } else if (this.toMove === 'black' && piece === BoardContent.BlackPawn) {
      return (
        fromPosition.isSouthTo(toPosition) && isPawnMove(fromPosition.y === 1)
      )
    }
